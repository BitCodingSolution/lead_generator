...
async function recordAccountWarning(phrase, sender) {
  ...
  // Forward to dashboard so the server-side safety rail also kicks in.
  // Best-effort: dashboard may be offline; don't fail the extension flow.
  try {
    ...
    await fetch(DASHBOARD_WARNING, {
      ...
      body: JSON.stringify({ phrase: String(phrase).slice(0, 200), url }),
    });
  } catch (e) {
    console.warn("Dashboard warning forward failed:", e.message);
  }
}
...
// ============================================================
// DASHBOARD (BitCoding LinkedIn API — replaces the legacy Google Sheet)
// ============================================================
//
// Old flow: extension → Apps Script webhook → Google Sheet row.
// New flow: extension → http://localhost:8900/api/linkedin/ingest → SQLite.
//
...
async function getDashboardKey() {
  ...
  // Allow users who still have the old sheetWebhookUrl field to migrate by
  // putting the dashboard key there too — Settings UI uses dashboardApiKey
  // as the canonical key, but we fall back so nothing breaks.
  return String(dashboardApiKey || sheetWebhookUrl || "").trim();
}

async function handleSaveToSheet(payload, _force) {
  if (!payload) throw new Error("No payload to save.");

  const key = await getDashboardKey();
  if (!key) {
    throw new Error(
      "Dashboard API key not set. Go to Settings → Dashboard and paste the key from /linkedin/settings."
    );
  }

  // Map legacy Apps Script payload → dashboard IngestPost shape.
  ...
  const lead = {
    ...
    rate:       payload.rate_budget || payload.rate || null,
  ...
  if (res.status === 401) {
    throw new Error(
      "Dashboard rejected the API key. Re-issue one at /linkedin/settings → Extension keys."
    );
  }
  if (!res.ok) {
    throw new Error(
      `Dashboard HTTP ${res.status}. ${data.detail || bodyText.slice(0, 200)}`
    );
  }

  // Dashboard response shape: { inserted, updated, dup_bin, blocked, total }.
  // The popup previously branched on { duplicate: true }; preserve that
  // contract so existing UI flow (ask to force-save) still works when the
  // post was already in the Recyclebin.
  if ((data.dup_bin || 0) > 0) {
    return {
      duplicate: true,
      matchType: "recyclebin",
      matchValue: lead.post_url,
      message: "This post was previously archived. Restore it from Recyclebin if you want to re-engage.",
    };
  }
  if ((data.blocked || 0) > 0) {
    return {
      duplicate: true,
      matchType: "blocklist",
      matchValue: lead.company || lead.email || "",
      message: "Blocked by your company/domain blocklist.",
    };
  }
...
async function handleTestSheet() {
  ...
  if (res.status === 401) {
    throw new Error("Invalid dashboard API key.");
  }
  if (!res.ok) {
  ...
  try {
    ...
    throw new Error(
      `Not JSON (is the backend running on :8900?). First 200 chars: ${snippet}`
    );
  }

  // Dashboard /overview returns { total, new, drafted, ... }. We just need
  // to confirm it's a shaped response.
  if (typeof data.total !== "number") {
    throw new Error(
      `Unexpected dashboard response. Got: ${JSON.stringify(data).slice(0, 200)}`
    );
  }
  return {
    service: "bitcoding-dashboard",
    version: "1.0",
    total: data.total,
