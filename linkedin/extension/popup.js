...
saveSheetBtn.addEventListener("click", async () => {
  ...
  // Validate endpoint if user typed something non-masked
  let endpointToSave = null;
  const isMasked = sheetWebhookInput.dataset.masked === "1" && /•/.test(endpointRaw);
  if (!isMasked && endpointRaw) {
    if (!/^https:\/\/script\.google\.com\/macros\/s\//.test(endpointRaw)) {
      setSheetStatus(
        "Save endpoint should start with https://script.google.com/macros/s/",
        true
      );
      return;
    }
  ...
  // Validate view URL if user typed something
  let viewToSave = null;
  let clearView = false;
  if (viewRaw) {
    if (!/^https:\/\/docs\.google\.com\/spreadsheets\//.test(viewRaw)) {
      setSheetStatus(
        "Open URL should start with https://docs.google.com/spreadsheets/",
        true
      );
      return;
    }
